...
class AccessibilityNotificationsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        customModalButton.setTitle(NSLocalizedString("Present Custom Modal (.screenChanged)", comment: ""), for: .normal)
        customToastButton.setTitle(NSLocalizedString("Present Custom Toast (.announcement)", comment: ""), for: .normal)
        
        NotificationCenter.default.addObserver(self, selector: #selector(handleReduceTransparency), name: UIAccessibility.reduceTransparencyStatusDidChangeNotification, object: nil)
        ...
        if let customToastView = CustomToastView.loadFromNib() {
            ...
            customToastView.configureWithTitle(NSLocalizedString("Custom Toast Shown!", comment: ""))
    ...
    private func presentToastView() {
        ...
        UIView.animate(withDuration: 0.2,
                       ...
                       completion: { _ in
                        ...
                        let announcementAttributedString = NSAttributedString(string: customToastView.accessibilityLabel!,
                                                                              attributes: queueAnnouncementAttributes)
    ...
    @IBAction func customModalButtonPressed(_ sender: Any) {
        presentModalSheetview()
    }
    
    @IBAction func customToastButtonPressed(_ sender: Any) {
        presentToastView()
    }
}
