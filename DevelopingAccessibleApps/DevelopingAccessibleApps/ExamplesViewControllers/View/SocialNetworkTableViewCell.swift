...
class SocialNetworkTableViewCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        let repostTitle = NSLocalizedString("Repost", bundle: .main, comment: "")
        let likeTitle = NSLocalizedString("Like", bundle: .main, comment: "")
        let shareTitle = NSLocalizedString("Share", bundle: .main, comment: "")
        ...
        accessibilityCustomActions = [
            UIAccessibilityCustomAction(name: repostTitle, target: self, selector: #selector(userButtonPressed(_:))),
    ...
    func configureWithViewModel(_ viewModel: SocialNetworkViewModel) {
        ...
        if UIAccessibility.isVoiceOverRunning,
            ...
            accessibilityLabel = "\(String(describing: userAccessibilityLabel)). " +
                "\(String(describing: postAccessibilityLabel))"
            accessibilityCustomActions?.append(
                UIAccessibilityCustomAction(
                    name: viewModel.userName, target: self, selector: #selector(userButtonPressed(_:))
                )
            )
        }
    }
    ...
    @objc func handleAction(_ action: UIAccessibilityCustomAction) -> Bool { true }

    @IBAction private func userButtonPressed(_ sender: Any) {
        print("User button presed")
    }

    @IBAction private func repostButtonPressed(_ sender: Any) {
